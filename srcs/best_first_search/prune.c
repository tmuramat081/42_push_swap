...
bool	is_empty_operation(t_node *node, t_operation next)
{
	...
	{
		if (next == OP_SA || next == OP_PA || next == OP_RA
			|| next == OP_RRA || next == OP_SS || next == OP_RR || next == OP_RRR)
			return (true);
	}
	else if (ft_deque_is_empty(node->stack_b) == true)
	{
		if (next == OP_SB || next == OP_PB || next == OP_RB
			|| next == OP_RRB || next == OP_SS || next == OP_RR || next == OP_RRR)
			return (true);
	}
